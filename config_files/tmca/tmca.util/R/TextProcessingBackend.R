...
QuantedaBackend = R6::R6Class(
  ...
  private = list(
    ...
    available_options = list(
      ...
      remove_custom = NULL,
      remove_custom_valueType= "fixed",
      remove_custom_case_insensitive=TRUE,
      save_custom = NULL,
      tolower = F,
    ...
    remove_custom = function(x) {
      if (!is.null(private$available_options$remove_custom)) {
        ...
        if (is.character(private$available_options$remove_custom) | is.list(private$available_options$remove_custom)) {
          ...
          #}else{
          x = x %>% quanteda::tokens_remove(private$available_options$remove_custom, valuetype=private$available_options$remove_custom_valueType,
                                            case_insensitive=private$available_options$remove_custom_case_insensitive)
          #}
